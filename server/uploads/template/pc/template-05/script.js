...
function showReplyInput(el) {
    var card = el.closest('.tb-reply-card');
    var wrap = card.querySelector('.tb-reply-wrap');
    var hidden = wrap.style.display === 'none';
    document.querySelectorAll('.tb-reply-wrap').forEach(function(w) { w.style.display = 'none'; });
    if (hidden) { wrap.style.display = 'block'; wrap.querySelector('textarea').focus(); }
}

async function publishReply(el) {
    var wrap = el.closest('.tb-reply-wrap');
...
async function submitReply() {
    var btn = pendingReply.el.closest('.tb-reply-wrap').querySelector('.tb-reply-submit');
    var orig = btn.innerHTML;
