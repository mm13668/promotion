  ...
  <div>
    ...
    <div class="gva-table-box">
      <el-table :data="tableData" row-key="ID" style="width:100%">
        ...
        <el-table-column prop="name" label="称号名称" min-width="200" />
    ...
    <el-drawer v-model="drawer" size="500" :show-close="false">
      <template #header>
        <div class="flex justify-between items-center">
          <span class="text-lg">{{ form.ID ? '编辑' : '新增' }}称号</span>
          <div>
...
const submit = async () => {
  if (!form.value.name) { ElMessage.error('请输入称号名称'); return }
  let res = form.value.ID ? await updateQATitle(form.value) : await createQATitle(form.value)
  if (res.code === 0) { ElMessage.success('保存成功'); drawer.value = false; getTableData() }
}
const remove = async (row) => {
  await ElMessageBox.confirm('确认删除该记录？', '提示')
  const res = await deleteQATitle({ ID: row.ID })
