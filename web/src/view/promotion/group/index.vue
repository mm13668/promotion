...
const form = ref({ ID: 0, name: '', regionId: 0, sort: 0, remark: '' })
const openForm = (row) => {
  if (row) {
    form.value = { ...row }
  } else {
    form.value = { ID: 0, name: '', regionId: 0, sort: 0, remark: '' }
  }
  drawer.value = true
}
...
const submit = async () => {
  ...
  let res
  if (form.value.ID) {
    res = await updatePromotionGroup(form.value)
  } else {
    res = await createPromotionGroup(form.value)
  }
  if (res.code === 0) {
...
const remove = async (row) => {
  ...
  const res = await deletePromotionGroup({ ID: row.ID })
  if (res.code === 0) {
